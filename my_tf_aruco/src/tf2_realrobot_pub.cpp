...
class Tf2Pub : public rclcpp::Node {
  ...
  void aruco_geometry_callback(const geometry_msgs::msg::TransformStamped::SharedPtr msg) {
    // msg is camera to aruco TransformStamped 
    ...
    std::string toFrameRel = "D415_color_optical_frame"; // TODO change Camera link name
    ...
    std::string fromFrameRel = "D415_color_optical_frame"; // TODO real robot is base_link
    std::string toFrameRel = "aruco_frame"; // TODO change Camera link name
    geometry_msgs::msg::TransformStamped msg_camera_aruco;
    rclcpp::Time now2 = this->get_clock()->now();
    msg_camera_aruco.header.stamp = now2;
    msg_camera_aruco.header.frame_id = fromFrameRel;
    msg_camera_aruco.child_frame_id = toFrameRel;
    msg_camera_aruco.transform.translation.x = msg->transform.translation.x;
    msg_camera_aruco.transform.translation.y = msg->transform.translation.y;
    msg_camera_aruco.transform.translation.z = msg->transform.translation.z;
    msg_camera_aruco.transform.rotation.x = msg->transform.rotation.x;
    msg_camera_aruco.transform.rotation.y = msg->transform.rotation.y;
    msg_camera_aruco.transform.rotation.z = msg->transform.rotation.z;
    msg_camera_aruco.transform.rotation.w = msg->transform.rotation.w;

...
#define Z_OFFSET                                                               \
  0.25 // best value 0.25,and atleast 0.25 because it measures from tool0

static const rclcpp::Logger LOGGER =
    rclcpp::get_logger("move_realrobot_hole_service");
static const std::string PLANNING_GROUP = "ur_manipulator";

class MoveitSimServerNode : public rclcpp::Node {
public:
  MoveitSimServerNode(rclcpp::NodeOptions &node_options)
      : Node("moveit_realrobot_hole_service_node", node_options) {
    ...
    srv_ = create_service<SetBool>(
        ...
        std::bind(&MoveitSimServerNode::moveit_realrobot_hole_callback, this,
                  _1, _2));
    ...
    // rviz_visual_tools::RvizVisualTools visual_tools_org("base_link",
    // "planning_scene_ros_api_tutorial", node_); visual_tools =
    // std::make_shared<rviz_visual_tools::RvizVisualTools>(visual_tools_org);
...
private:
  ...
  void setup_waypoints_target(float target_x, float target_y, float target_z) {
    ...
    // target_pose_robot_ = move_group->getCurrentPose().pose; this is ok, but
    // trying below may be better
    target_pose_robot_.position.x =
        move_group->getCurrentPose()
            .pose.position.x; // start with current, and waypont to real target
    target_pose_robot_.position.y =
        move_group->getCurrentPose().pose.position.y;
    ...
    // cartesian_waypoints_.push_back(target_pose_robot_);
    // RCLCPP_INFO(LOGGER, "setup_waypoints_target(%f, %f, %f)",x_delta,
    // y_delta, z_delta);
  }
  ...
  void moveit_realrobot_hole_callback(
    ...
    float z_offset = Z_OFFSET; // hard code this is the distance from tool0 to
                               // from end_effector_tip_link
    ...
    float x_diff = tf_hole_to_base_link.transform.translation.x -
                   move_group->getCurrentPose().pose.position.x;
    float y_diff = tf_hole_to_base_link.transform.translation.y -
                   move_group->getCurrentPose().pose.position.y;
    float z_diff = tf_hole_to_base_link.transform.translation.z -
                   move_group->getCurrentPose().pose.position.z + z_offset;

    if (how_far_from_target(x_diff, y_diff, z_diff) >
        far_threshold) { // is far away
    ...
    setup_waypoints_target(tf_hole_to_base_link.transform.translation.x,
                           tf_hole_to_base_link.transform.translation.y,
                           tf_hole_to_base_link.transform.translation.z +
                               z_offset);
...
int main(int argc, char *argv[]) {
  ...
  auto move_group_node_std =
      std::make_shared<MoveitSimServerNode>(node_options);
